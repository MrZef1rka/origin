...
void MainWindow::StartRace(void){
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto runTask = [this](ExampleRace* race) {
            race->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
        };

        // Запускаем и ждем завершения первого
        QtConcurrent::run(runTask, concurRace1).waitForFinished();

        // Запускаем и ждем завершения второго
        QtConcurrent::run(runTask, concurRace2).waitForFinished();
